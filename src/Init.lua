...
function WindUI:ToggleAcrylic(Value)
	if WindUI.Window and WindUI.Window.AcrylicPaint and WindUI.Window.AcrylicPaint.Model then
		...
		WindUI.Window.AcrylicPaint.Model.Transparency = Value and 0.2 or 0.4
		...
		if mainUI and mainUI.Main then
			...
			if bg then
				Creator.Tween(bg, 0.2, {
					ImageTransparency = Value and 0.2 or 0  -- Keep solid
				}):Play()
			end
			...
			if mainUI.MainBar then
				...
				if panelBg then
					Creator.Tween(panelBg, 0.2, {
						ImageTransparency = Value and 0.2 or 0.4  -- Darker to match sidebar
					}):Play()
				end
			end
		end
